...
const isKeyValid = computed(() => form.value.key === null || form.value.key.length > 3);
const isFormValid = computed(() => form.value.url.length > 0 && isValidUrl(form.value.url) && isKeyValid.value);
</script>
...
<template>
  ...
  >
    <div class="card-body">
      ...
      <div class="flex flex-wrap sm:flex-nowrap justify-between items-middle gap-5">
        ...
        <div class="form-group w-full sm:w-4/12">
          ...
          <input type="checkbox" :checked="form.key !== null" @change="toggleKey">
          <input
              v-model="form.key"
              class="input input-bordered w-full max-w-xs block"
              for="key"
              ...
              min="6"
          />
          <span v-if="errors.key" class="text-xs py-1 text-red-500">{{ errors.key }}</span>
        </div>
